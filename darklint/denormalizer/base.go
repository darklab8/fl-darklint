...
func (denormalizer *BaseDenormalizer) ReadBaseNames(marketconfig *market_mapped.Config, universeConfig *universe_mapped.Config, infocards *infocard.Config) {
	for _, base := range universeConfig.Bases {

		base_strid_name := int(base.StridName.Get())
		record, ok := infocards.Infonames[base_strid_name]
		if !ok {
		...
		denormalizer.baseGoods[base.Nickname.Get()].name = string(record)
	}
}
...
func (denormalizer *BaseDenormalizer) ReadRecycle(marketconfig *market_mapped.Config, universeConfig *universe_mapped.Config, systems *systems_mapped.Config) {
	for _, base := range universeConfig.Bases {
		...
		_, ok = system.BasesByBases[base.Nickname.Get()]
		if !ok {
